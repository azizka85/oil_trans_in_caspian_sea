...
void Solver::setDZ(double val) {
    if (val <= 0) {
        throw runtime_error(
            format("DZ should be > 0, but it is {}", val)
        );
    }
...
path Solver::createDirectory() {
    auto dirPath = path(
        format("{}/f={}, H={}/nu={}, g={}, rho={}/kb={}, qx={}, qy={}/dx={}, dy={}, dz={}", dir, f, h, nu, g, rho, kb, qx, qy, dx, dy, dz)
    );
    ...
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
            for (int k = 1; k < nz-1; k++) {
                ud[i][j][k] = uf[i][j][k] + f*dt*vf[i][j][k] + nu*dt*(uf[i][j][k+1] - 2*uf[i][j][k] + uf[i][j][k-1])/2/h/h/dz/dz 
                                + kb*dt*(uf[i][j][nz-1] + ua[i][j])/h - qx*dt/rho/h;

                vd[i][j][k] = vf[i][j][k] - f*dt*uf[i][j][k] + nu*dt*(vf[i][j][k+1] - 2*vf[i][j][k] + vf[i][j][k-1])/2/h/h/dz/dz 
                                + kb*dt*(vf[i][j][nz-1] + va[i][j])/h - qy*dt/rho/h;
            }

            ud[i][j][0] = uf[i][j][0] + f*dt*vf[i][j][0] + nu*dt*(uf[i][j][1] - uf[i][j][0])/h/h/dz/dz + kb*dt*(uf[i][j][nz-1] + ua[i][j])/h
                            - qx*dt/rho/h + 2*qx*dt/rho/h/dz;
            ud[i][j][nz-1] = uf[i][j][nz-1] + f*dt*vf[i][j][nz-1] + nu*dt*(-(1 + kb*h*dz/nu)*uf[i][j][nz-1] + uf[i][j][nz-2])/h/h/dz/dz
                            + kb*dt*(uf[i][j][nz-1] + ua[i][j])/h - qx*dt/rho/h - kb*dt*(u1a[i][j] + ua[i][j])/h/dz;

            vd[i][j][0] = vf[i][j][0] - f*dt*uf[i][j][0] + nu*dt*(vf[i][j][1] - vf[i][j][0])/h/h/dz/dz + kb*dt*(vf[i][j][nz-1] + va[i][j])/h
                            - qy*dt/rho/h + 2*qy*dt/rho/h/dz;
            vd[i][j][nz-1] = vf[i][j][nz-1] - f*dt*uf[i][j][nz-1] + nu*dt*(-(1 + kb*h*dz/nu)*vf[i][j][nz-1] + vf[i][j][nz-2])/h/h/dz/dz
                            + kb*dt*(vf[i][j][nz-1] + va[i][j])/h - qy*dt/rho/h - kb*dt*(v1a[i][j] + va[i][j])/h/dz;
        }
    }
}
...
void Solver::solve() {
    ...
    double dt = min(dtMax, h*dz*h*dz/2/nu);
    ...
    double al = -dt*nu/2/h/h/dz/dz;
    double al1 = -dt*nu/h/h/dz/dz;

    double ac = 1 + dt*nu/h/h/dz/dz;
    double ac0 = 1 + dt*nu/h/h/dz/dz;
    double ac1 = 1 + dt*nu/h/h/dz/dz + dt*kb/h/dz;

    double ar = -dt*nu/2/h/h/dz/dz;
    double ar0 = -dt*nu/h/h/dz/dz;
    ...
    while (t <= endTime) {
        ...
        for (int i = 0; i < nx; i++) {
            for (int j = 0; j < ny; j++) {
                Tridiagonal::solve(al, al1, ac, ac0, ac1, ar, ar0, ud[i][j], uf[i][j]);
                Tridiagonal::solve(al, al1, ac, ac0, ac1, ar, ar0, vd[i][j], vf[i][j]);                
            }
        }
